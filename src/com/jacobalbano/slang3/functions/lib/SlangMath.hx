...
class SlangMath
{
	public static function bind(scope:Scope):Void
	{
		...
		scope.functions.set("add", new NativeFunction(__add, 2, FunctionType.Function));
		
		scope.functions.set("-", new NativeFunction(__sub, 2, FunctionType.Function));
		scope.functions.set("sub", new NativeFunction(__sub, 2, FunctionType.Function));
		
		scope.functions.set("*", new NativeFunction(__mul, 2, FunctionType.Function));
		scope.functions.set("mul", new NativeFunction(__mul, 2, FunctionType.Function));
		
		scope.functions.set("/", new NativeFunction(__div, 2, FunctionType.Function));
		scope.functions.set("div", new NativeFunction(__div, 2, FunctionType.Function));
		
		scope.functions.set("%", new NativeFunction(__mod, 2, FunctionType.Function));
		scope.functions.set("mod", new NativeFunction(__mod, 2, FunctionType.Function));
	}
